...
def run_module(input_path_dict,output_folder_path):
    ...
    """
    cwd = os.getcwd()
    with open(input_path_dict['Raw CT Images']) as f:
        lines = f.read()
        scans_path = lines.split('\n',1)[0]

    with open(input_path_dict['Ground Truth Images']) as f:
        lines = f.read()
        seg_path = lines.split('\n',1)[0]
    ...
    segFiles=[]
    scanFiles = []
    segFiles+=[d for d in os.listdir(os.path.join(cwd,seg_path)) if 'Final' in d and '.nii.gz' in d] 
    scanFiles += [d for d in os.listdir(os.path.join(cwd,scans_path)) if '.nii.gz' in d]
    count=len(segFiles)
...
if __name__ == '__main__':
    ...
    cwd = os.getcwd()
    print(cwd)
    input_path_dict['Raw CT Images'] = 'input.txt'
    input_path_dict['Ground Truth Images'] = 'gt.txt'
